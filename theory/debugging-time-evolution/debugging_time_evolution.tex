...
\begin{equation}
    ...
    - \nabla^2 Q
\end{equation}
Calculating the elastic term explicitly yields:
\begin{equation}
    -\nabla^2 Q
    =
    S_0 \epsilon k^2 \sin kx
    \begin{bmatrix}
        0 &1 &0 \\
        1 &0 &0 \\
        0 &0 &0
    \end{bmatrix}
\end{equation}
Further, we may explicitly calculate the inner product of $\delta Q$ with each of those terms.
Because $Q$ and $\Lambda$ are diagonal, it happens that $Q:\delta Q = \Lambda : \delta Q = 0$.
Then we just calculate:
\begin{equation}
    -\nabla^2 Q : \delta Q
    =
    2 S_0^2 \epsilon^2 k^2 \sin^2 kx
\end{equation}
