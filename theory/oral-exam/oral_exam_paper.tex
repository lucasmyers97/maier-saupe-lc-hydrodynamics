...
\begin{document}
  ...
  In \ref{fig:cholesteryl-benzoate}, the molecule is completely anisotropic and so we may
  ...
  Now, these types of molecules tend to have three types of purely orientational
  patterns: polar, nematic, or isotropic -- some form spatial patterns, but we
  neglect those here.
  Schematics of these alignment types are given in \ref{fig:alignment-cartoons}
  \begin{figure}[h] 
    \centering
    ...
    \hfill
    \begin{subfigure}{0.3\textwidth}
      \includegraphics[width=\textwidth]{figures/polar_cartoon.png}
      \caption{}
      \label{fig:polar-cartoon}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.3\textwidth}
    ...
    \end{subfigure}
    \caption{Schematic representation of (a) isotropic,
      (b) polar, and (c) nematic alignment
      patterns of molecules. Cartoons taken from (REFERENCE SELINGER)}
  ...
  \begin{equation*}
    M = \langle r \rangle
  \end{equation*}
  where here $r$ is a unit vector assigned to each molecule as in \ref{fig:cholesteryl-benzoate}
	In the isotropic case this evaluates to zero for all components since, in the
  limit of large system size, for any particular molecule with orientation $r$ we
  will be able to find another molecule with orientation $-r$.
  ...
  order parameter:
  \begin{equation}
    T = \langle r \otimes r \rangle
  \end{equation}
  
  In the isotropic case, we must have that $\langle r_x^2 \rangle = \langle
  r_y^2 \rangle = \langle r_z^2 \rangle$ essentially by definition of the
  isotropic state.
  Further, we have that:
  \begin{equation}
    \langle r \cdot r \rangle
    = \langle r_x^2 \rangle + \langle r_y^2 \rangle + \langle r_z^2 \rangle
    = 1
  \end{equation}
  Hence the diagonal elements are just $1/3$.
  Additionally, for the off-diagonal elements in the thermodynamic limit, for
  every vector $(r_x, r_y, r_z)$ we may find a corresponding $(r_x, r_y, -r_z)$
  ...
  $r = (0, 0, \pm 1)$, we may explicitly calculate this tensor as:
  \begin{equation}
    T^\text{nem}
    =
    \begin{pmatrix}
  ...
  \begin{equation} \label{eq:Q-def}
    Q = \langle r \otimes r \rangle - \frac13 I
  \end{equation}
  ...
  \begin{equation}
    ...
    - (q_1 + q_2) (l \otimes l)
  \end{equation}
  Now, in the eigenbasis we have that $n\otimes n +
  m \otimes m + l \otimes l = I$ which is, in
  fact, basis independent.
  Hence we may add and subtract $\frac12 q_1 I$ to our expression to get:
  \begin{equation}
    Q
    =
    \tfrac32 q_1 (n\otimes n - \tfrac13 I)
    + (\tfrac12 q_1 + q_2) (m \otimes m)
    - (\tfrac12 q_1 + q_2) (l \otimes l)
  \end{equation}
  Defining $S = \frac32 q_1$, and $P = \frac12 {q_1 + q_2}$ we explicitly see our
  five degrees of freedom:
  \begin{equation} \label{eq:Q-orthogonal-decomposition}
    Q
    = S (n \otimes n - \tfrac13 I)
    + P (m \otimes m - l \otimes l)
  \end{equation}
  ...
  along one axis.
  If we calculate $\langle r \otimes r \rangle - \tfrac13 I$, we get:
  \begin{equation} \label{eq:uniaxial-Q-decomposition}
    \begin{split}
      \langle r \otimes r \rangle - \tfrac13 I
      &=
      \left( \langle \cos^2 \theta \rangle - \tfrac13 \right) n\otimes n
      + \left( \tfrac12 \langle \sin^2\theta  \rangle - \tfrac13 \right) m \otimes m
      + \left( \tfrac12 \langle \sin^2\theta \rangle - \tfrac13 \right) l \otimes l\\
      &= 
      \left< \cos^2 \theta - \tfrac13 \right> n \otimes n
      - \tfrac12 \left< \cos^2\theta - \tfrac13 \right> m \otimes m
      - \tfrac12 \left< \cos^2\theta - \tfrac13 \right> l \otimes l\\
    \end{split}
  \end{equation}
  ...
  Thus, $S$ characterizes the degree of alignment and $n$ -- sometimes called
  ...
  HERE I NEED TO DO A CALCULATION SHOWING THAT LDG BLOWS UP WITH ANISOTROPIC
  ELASTICITY

  \subsection{Topological defects}
  HERE I WANT TO BRIEFLY EXPLAIN WHAT DEFECTS LOOK LIKE AND WHAT THEY ARE
