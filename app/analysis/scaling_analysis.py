...
if __name__ == "__main__":
    ...
    large_sim = prof_data[prof_data['refines'] == 6]
    ...
    ax.set_yscale("log")
    ax.set_title("Scaling for 1,250,235 degrees of freedom")
    ax.set_xlabel("Number of processors")
    ax.set_ylabel("Wall time (s)")
    ax.legend()
    plt.tight_layout()
    fig.savefig("walltime_large_simulation.png")

    fig, ax = plt.subplots()
    ax.plot(num_cores, cpu_times, marker='o')
    ax.set_ylim(0, cpu_times[-1]*1.1)
    ax.set_title("CPU time scaling for 1,250,235 DoFs")
    ax.set_xlabel("Number of processors")
    ax.set_ylabel("Total CPU time (across all processors)")
    plt.tight_layout()
    fig.savefig("cputime_large_simulation.png")
    ...
    ax.set_xticks([2, 4, 8, 16, 32, 64, 128, 256, 512],
                  labels=[2, 4, 8, 16, 32, 64, 128, 256, 512])
    ax.set_yscale("log")
    ax.set_title("CPU time / core scaling for 1,250,235 DoFs")
    ax.set_xlabel("Number of processors")
    ax.set_ylabel("CPU time per core")
    ax.legend()
    plt.tight_layout()
    fig.savefig("cputime_per_core_large_simulation.png")
