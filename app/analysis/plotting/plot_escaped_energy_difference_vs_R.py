...
def main():
    ...
    for L3 in L3_values:
        ...
        ax.plot(L2, normalized_diff, #linestyle='', 
                marker='o', label=r'$L_3 = {}$'.format(L3))
        ax.set_xlabel(r'$L_2$')
        ax.set_ylabel(r'$\frac{F_\text{TER} - F_\text{ER}}{\left|F_\text{TER} + F_\text{ER}\right|}$')

        # if elastic:
        #     ax.set_title(r'Escaped elastic energy diff \\$R = {}, L_3 = {}$'.format(R, L3), wrap=True)
        # else:
        #     ax.set_title(r'Escaped energy diff \\$R = {}, L_3 = {}$'.format(R, L3))
        # if elastic:
        #     ax.set_title(r'Escaped elastic energy diff $R = {}$'.format(R), wrap=True)
        # else:
        #     ax.set_title(r'Escaped energy diff $R = {}$'.format(R))
