...
def main():
    ...
    energy_range = min_data['energy per length'].max() - min_data['energy per length'].min()
    energy_range_exponent = np.floor(np.log10(np.abs(energy_range))).astype(int)
    offset = np.round(min_data['energy per length'].min() - energy_range, decimals=-energy_range_exponent)

    max_energy_range = 0
    for L2 in L2_vals:
        range_data = data[data['L2'] == L2]
        energy_range = range_data['energy per length'].max() - range_data['energy per length'].min()
    ...
    for L2, i in zip(reversed(L2_vals), range(ax.shape[0])):
        ...
        ax[i].plot(plot_data['alpha'], plot_data['energy per length'], ls='', marker=markers[i], color=colors[i], label=r'$L_2 = {}$'.format(L2))
        if i == len(L2_vals) - 1:
            ax[i].plot(min_x, min_y, marker='x', ls='', label=r'$\omega_0$')
        else:
        ...
        actual_range = plot_data['energy per length'].max() - plot_data['energy per length'].min()
        room = max_energy_range - actual_range
        ax[i].set_ylim(bottom=plot_data['energy per length'].min() - room*0.5,
                       top=plot_data['energy per length'].max() + room*0.5)
    ...
    ax[-1].set_xticks([np.pi / (8 * R), 2 * np.pi / (8 * R)], 
                      [r'$\pi / 8 R$', r'$\pi / 4 R$'])
