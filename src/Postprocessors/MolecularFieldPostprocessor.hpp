#ifndef MOLECULAR_FIELD_POSTPROCESSOR_HPP
#define MOLECULAR_FIELD_POSTPROCESSOR_HPP
...
template <int dim>
class MolecularFieldPostprocessor : public dealii::DataPostprocessor<dim>
{
public:
    MolecularFieldPostprocessor(const LagrangeMultiplierAnalytic<dim> lma_,
                           const double alpha_)
        : lma(lma_)
        , alpha(alpha_)
    {};
...
template <int dim>
dealii::UpdateFlags MolecularFieldPostprocessor<dim>::get_needed_update_flags() const
{
...
template <int dim>
std::vector<std::string> MolecularFieldPostprocessor<dim>::get_names() const
{
...
MolecularFieldPostprocessor<dim>::get_data_component_interpretation()
  const
{
...
template <int dim>
void MolecularFieldPostprocessor<dim>::evaluate_vector_field
(const dealii::DataPostprocessorInputs::Vector<dim> &inputs,
 std::vector<dealii::Vector<double>> &computed_quantities) const
{
    ...
    for (unsigned int q = 0; q < n_quadrature_points; ++q)
    {
        ...
        computed_quantities[q](0) = H[0][0];
        computed_quantities[q](1) = H[0][1];
        computed_quantities[q](3) = H[1][1];
        if (dim == 3)
        {
            computed_quantities[q](2) = H[0][2];
            computed_quantities[q](4) = H[1][2];
        }
    }
}
